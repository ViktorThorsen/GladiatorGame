...
public class ReplayInventoryBattleHandler : MonoBehaviour
{
    ...
    private ReplayPlayerMovement playerMovement;
    ...
    void Start()
    {
        ...
        thisUnit = gameObject;
        playerMovement = thisUnit.GetComponent<ReplayPlayerMovement>();
    ...
    private void ResetCombatInventory()
    {
        ...
        List<string> weaponNames = ReplayManager.Instance.selectedReplay.player.weapons.weaponNames;
        List<string> consumableNames = ReplayManager.Instance.selectedReplay.player.consumables.consumableNames;
    ...
    public IEnumerator EquipWeaponAndStartMoving(float delay)
    {
        ...
        if (itemToEquip != null && itemToEquip.itemSprite != null)
        {
            ...
            if (handSpriteRenderer != null)
            {
                ...
                ReplayCharacterData.Instance.AddEquipStats(
    ...
    private (Item item, int index) GetWeaponFromReplayShortcut(int shortcutSlot)
    {
        ...
        var weaponEquipAction = replay.actions
            .Where(a => a.Turn == currentTurn &&
                        ...
                        a.Actor == CharacterType.Player) // eller Player beroende på vem det gäller
            .FirstOrDefault();

...
public class ReplayEnemyPetMovement : MonoBehaviour
{
        ...
        if (secondAttackControll2 != null)
        {
            ...
            else
            {
                if (enemy.tag == "Player")
                {
                    ...
                    var attack = GetNthAttackThisTurn(GetCharacterType(pet.tag), 2); // eller Pet2/Pet3 beroende på vilket pet
                    if (attack == null)
                    {
                        Debug.LogWarning("❌ Ingen andra attack hittades i denna rundan.");
                    }
            ...
            if (secondAttackControll3 != null)
            {
                ...
                else
                {
                    if (enemy.tag == "Player")
                    {
                        ...
                        var attack = GetNthAttackThisTurn(GetCharacterType(pet.tag), 3); // eller Pet2/Pet3 beroende på vilket pet
                        if (attack == null)
                        {
                            Debug.LogWarning("❌ Ingen andra attack hittades i denna rundan.");
                        }
                ...
                if (secondAttackControll4 != null)
                {
                    ...
                    else
                    {
                        if (enemy.tag == "Player")
                        {
                            ...
                            var attack = GetNthAttackThisTurn(GetCharacterType(pet.tag), 4); // eller Pet2/Pet3 beroende på vilket pet
                            if (attack == null)
                            {
                                Debug.LogWarning("❌ Ingen andra attack hittades i denna rundan.");
                            }
    ...
    bool RollForEnemy()
    {
        ...
        if (enemy.tag == "Player")
        {
            enemyPlayerMovement = enemy.GetComponent<ReplayPlayerMovement>();
        }
        else if (enemy.tag.StartsWith("Pet"))
        {
            enemyPetMovement = enemy.GetComponent<ReplayPetMovement>();
        }
